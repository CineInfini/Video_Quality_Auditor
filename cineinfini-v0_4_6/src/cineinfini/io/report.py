...
def _load_audit_data(intra_report_dir: Path) -> dict | None:
    """Load data.json from one intra-report directory. Returns None if missing."""
    json_path = Path(intra_report_dir) / "data.json"
...
def _aggregate_per_video_metrics(intra_report_dir: Path, data: dict) -> dict:
    """Compute per-video metric means from a single audit's gates."""
    ...
    return {
        "video":          Path(intra_report_dir).name,
        "motion_mean":    _mean("motion_peak_div"),
        "ssim_mean":      _mean("ssim3d_self"),
        "flicker_mean":   _mean("flicker"),
        "identity_mean":  _mean("identity_intra"),
        "ssim_lr_mean":   _mean("ssim_long_range"),
        "clip_temp_mean": _mean("clip_temp_consistency"),
...
def generate_inter_report(intra_report_dirs, output_dir, thresholds, comparison_name="comparison"):
    ...
    # Stage 1: load + aggregate (extracted helpers, CC ≤ 4 each)
    rows = []
    for d in intra_report_dirs:
        data = _load_audit_data(d)
        if data is None:
            continue
        rows.append(_aggregate_per_video_metrics(d, data))
