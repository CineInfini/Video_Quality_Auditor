...
def compute_inter_shot_coherence(
    ...
    *,
    ...
    1) v0.3.0 audit.py style (positional):
         compute_inter_shot_coherence(
             shot_repr_frames, detector, embedder, clip_scorer, subsample
         )
    2) Pre-v0.2.0 explicit (4 positional, like the original tarball):
         compute_inter_shot_coherence(
             shot_frames, clip_model, clip_preprocess, device
         )
    3) Kwargs only.
    ...
    list of dicts with keys:
        shot_a, shot_b, structure, style, semantic, total, clip_available
        + identity_dtw_inter, identity_dtw_n  (when DTW prerequisites met)
    """
    # Detect call style by sniffing types of the second positional arg
    ...
    if arg2 is not None and hasattr(arg2, "detect"):
        # Style 1: (shot_frames, detector, embedder, clip_scorer, subsample)
    ...
    if isinstance(shot_frames, list):
        ...
        raise TypeError(
            f"shot_frames must be list or dict, got {type(shot_frames).__name__}"
        )

    # Optional DTW backend lookup
    _dtw_fn = None
    ...
    for i in range(len(shot_ids) - 1):
        ...
        # Style (HSV histogram correlation)
        ...
        # Optional v0.3.0: inter-shot DTW identity drift
        if _dtw_fn is not None:
            try:
                dtw_res = _dtw_fn(
                    frames_a, frames_b, detector, embedder, max_samples=8
                )
...
def compute_narrative_coherence(shot_frames, dinov2_model, dinov2_processor, device):
    """Cosine similarity between DINOv2 embeddings of mid-frames of adjacent
    shots. Returns None when DINOv2 is not loaded (no silent fake-perfect)."""
