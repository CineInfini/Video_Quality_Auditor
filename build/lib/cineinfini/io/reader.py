...
def detect_shot_boundaries(video_path, max_duration_s, shot_threshold, min_shot_duration_s, downsample_to,
                           ...
                           hist_resize=None, bins_hue=None, bins_sat=None):
    cfg = get_config()
    proc = cfg.processing
    if hist_resize is None:
        hist_resize = tuple(proc.get("hist_resize", (160, 90)))
    if bins_hue is None:
        bins_hue = int(proc.get("hist_bins_hue", 20))
    if bins_sat is None:
        bins_sat = int(proc.get("hist_bins_sat", 20))
