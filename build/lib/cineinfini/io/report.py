...
def safe_format(v, fmt=".3f"):
    """Return a formatted value or '—' if None."""
    return f"{v:{fmt}}" if v is not None else "—"

def generate_intra_report(video_name, metrics_data, output_dir, thresholds=None):
    if thresholds is None:
        thresholds = dict(get_config().thresholds)
...
def generate_inter_report(intra_report_dirs, output_dir, thresholds=None, comparison_name="comparison"):
    if thresholds is None:
        thresholds = dict(get_config().thresholds)
