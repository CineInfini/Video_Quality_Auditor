"""Shared video processing context: ModelPool, VideoInfoLite, VideoContext.

ModelPool lazily caches heavy models (ArcFace, CLIP, DINOv2) so they are
loaded once per audit and shared across modules. VideoContext bundles the
parsed shots, per-shot frame buffers and the resolved configuration.
"""
...
class ModelPool:
    """Lazy, per-key cache for ML models with timing instrumentation."""

    def __init__(self, device: str = "cpu") -> None:
        self.device = device
        self._cache: Dict[str, Any] = {}
        self._timing: Dict[str, float] = {}
        self._loaded_devices: Dict[str, str] = {}
    ...
    def get_or_load(self, key: str, loader: Callable[[], Any]) -> Optional[Any]:
        ...
        try:
            ...
            logger.info(
                "ModelPool: loaded '%s' in %.2fs (device=%s)",
                key, self._timing[key], self._loaded_devices[key],
            )
            return obj
    ...
    def to_device(self, key: str, device: Optional[str] = None) -> Optional[Any]:
        ...
        try:
            if hasattr(obj, "to"):
                obj = obj.to(device)
                self._cache[key] = obj
                self._loaded_devices[key] = device
        except Exception:
            pass
        return obj
...
class VideoContext:
    ...
    def get_shot_frames(self, shot_id: int) -> List[Any]:
        return self.shot_frames.get(shot_id, [])

    def shot_ids(self) -> List[int]:
        return sorted(self.shot_frames.keys())

    def iter_shot_batches(
        self, batch_size: Optional[int] = None
    ) -> Iterator[Tuple[List[int], List[Any]]]:
        from .device_utils import effective_batch_size

        bs = batch_size or effective_batch_size(default=16)
        ids: List[int] = []
        frames: List[Any] = []
